...
command -v bun >/dev/null 2>&1 || { echo "FATAL: bun is required for the Astro stack" >&2; exit 1; }
...
setsid nohup bunx astro dev --host 0.0.0.0 --port "$local_port" \
  > "$APP_DEST/log/astro-server.log" 2>&1 &
echo "$!" > "$pidfile"
...
if [[ "$healthy" -ne 1 ]]; then
  echo "FATAL: Astro app did not report healthy within ${timeout}s" >&2
  tail -100 "$APP_DEST/log/astro-server.log" >&2 || true
  exit 1
fi

printf '%s\n' "Astro app healthy on port $local_port"

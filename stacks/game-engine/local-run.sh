...
command -v bun >/dev/null 2>&1 || { echo "FATAL: bun is required for the game-engine stack" >&2; exit 1; }
...
mkdir -p tmp/pids log build/web
...
HOST_PORT="$local_port" setsid nohup bun server.ts \
  > "$APP_DEST/log/game-server.log" 2>&1 &
echo "$!" > "$pidfile"
...
if [[ "$healthy" -ne 1 ]]; then
  echo "FATAL: game-engine app did not report healthy within ${timeout}s" >&2
  tail -80 "$APP_DEST/log/game-server.log" >&2 || true
  exit 1
fi

printf '%s\n' "game-engine app healthy on port $local_port"

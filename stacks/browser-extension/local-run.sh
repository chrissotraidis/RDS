...
command -v bun >/dev/null 2>&1 || { echo "FATAL: bun is required for the browser-extension stack" >&2; exit 1; }
...
HOST_PORT="$local_port" setsid nohup bun preview-server.ts \
  > "$APP_DEST/log/extension-preview.log" 2>&1 &
echo "$!" > "$pidfile"
...
if [[ "$healthy" -ne 1 ]]; then
  echo "FATAL: browser-extension preview did not report healthy within ${timeout}s" >&2
  tail -80 "$APP_DEST/log/extension-preview.log" >&2 || true
  exit 1
fi

printf '%s\n' "browser-extension preview healthy on port $local_port"
